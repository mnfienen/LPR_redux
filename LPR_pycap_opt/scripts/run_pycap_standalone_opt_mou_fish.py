...
def instantiate():
    ...
    return initial_dict, bdplobs, fishcurve, ref_flow

def get_results(pars,  obsnames, initial_dict, bdplobs,  fishcurve, ref_flow, write_csv= False):
    ...
    bdf.loc['fish_prob'] = [
        np.interp(8.6 - bdf.loc['lpr:total_combined:bdpl', 'value'],
                  fishcurve.index,
                  fishcurve.values),
                  'fish_prob'
    ]
...
def process_realization(args):
    creal, row, obs_names, initial_dict, bdplobs,  par_names, fishcurve, ref_flow = args
    allout = get_results(row[par_names], obs_names, initial_dict, bdplobs, fishcurve, ref_flow)
    return creal, allout.loc[obs_names].to_numpy()

def standalone_worker(initial_dict, bdplobs, fishcurve, ref_flow, rootname='mou_fish'):
    # Read RNS run data
    ...
    args_list = [
        (creal, runs_df.loc[creal], obs_names, initial_dict, bdplobs, par_names, fishcurve, ref_flow)
...
if __name__== "__main__":
    initial_dict, bdplobs, fishcurve, ref_flow = instantiate()
    standalone_worker(initial_dict, bdplobs, fishcurve, ref_flow,rootname=sys.argv[1])
